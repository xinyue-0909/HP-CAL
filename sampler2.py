...
def oracle_labeling(superpoint_inds, components, input_gt, pseudo_gt, cloud_name, w, sampler_args, prob_class, threshold, budget, min_size, total_obj, ply_data):
    ...
    '''
    sp_neighbour_path = '/data3/ncl/Semantic3D/0.012/spNeighbour'#邻居超点路径
    sp_neighbour_file = os.path.join(sp_neighbour_path, cloud_name + '_25spNeighbour.pkl')#邻居超点文件
        ...
        if "divide" in sampler_args:
            ...
            for superpoint_idx in superpoint_inds:#遍历超点
                if budget["click"] > 0:#如果还有标注预算
                    ...
                    if len(point_inds) >= min_size:
                        ...
                        # 对其邻居节点进行合并标注
                            ...
                            for sp_neighbour_idx in sp_neighbours_idx:
                                ...
                                w2 = 1/ (1+abs(rgb_sim))
                                if (w1 * abs(vector_cosine_similarity) + w2 * abs(rgb_sim) < 0.990 ):
                    ...
                    break   
        elif "change_sim3" in sampler_args:
            ignore_merge_vector_sp_count = 0
            ignore_merge_rgb_sp_count = 0
            ignore_merge_vector_and_rgb_sp_count = 0
            ignore_purity_sp_count = 0
            # model_path = "/home/ncl/projects/SSRD_AL_semantic3d/fine_tune_model_2/superpoint_annotation_2.pth"
            model_path = "/home/ncl/projects/SSDR-AL-main/SSRD_AL_semantic3d/fine_tune_model_3/superpoint_annotation_3.pth"
            class_model =  MLP(3)  # 先初始化模型
            class_model.load_state_dict(torch.load(model_path))
            class_model.eval()  # 切换到评估模式
            vector_sim = []
            color_sim = []
            final_label = []
            for superpoint_idx in superpoint_inds:#遍历超点
                if budget["click"] > 0:#如果还有标注预算
                    ...
                    if len(point_inds) >= min_size:
                        ...
                        # 对其邻居节点进行合并标注
                            ...
                            for sp_neighbour_idx in sp_neighbours_idx:
                                ...
                                w1 = 1/ (1 + abs(vector_cosine_similarity))
                                w2 = 1/ (1+abs(rgb_sim))
                                class_sim_score = w1 * abs(vector_cosine_similarity) + w2 * abs(rgb_sim)
                                # if (class_sim_score < 0.99 ):
                                #     ignore_merge_vector_and_rgb_sp_count += 1
                                #     vector_sim.append([vector_cosine_similarity,rgb_sim, class_sim_score])
                                ...
                                with torch.no_grad():  # 关闭梯度计算，加速推理
                                    data = [vector_cosine_similarity, rgb_sim, class_sim_score]
                                    inputs = torch.tensor(data, dtype=torch.float32)
                                    outputs = class_model(inputs)  # 获取预测结果
                                    # print(outputs.shape)
                                    # print(data)
                                    # print(outputs.unsqueeze(0))
                                    outputs = outputs.unsqueeze(0)
                                    # print(outputs.shape)
                                    prediction = torch.argmax(outputs, dim=1)  # 取最大索引
                                    # predicted = torch.argmax(outputs, dim=-1)  # 适用于分类任务
                                    # print(outputs)
                                    # print(prediction)
                    ...
                    break 
        elif "ablation2" in sampler_args:
            ignore_merge_vector_sp_count = 0
            ignore_merge_rgb_sp_count = 0
            ignore_merge_vector_and_rgb_sp_count = 0
            ignore_purity_sp_count = 0
            for superpoint_idx in superpoint_inds:#遍历超点
                if budget["click"] > 0:#如果还有标注预算
                    ...
                    if len(point_inds) >= min_size:
                        ...
                        # 对其邻居节点进行合并标注
                            ...
                            for sp_neighbour_idx in sp_neighbours_idx:
                                ...
                                w1 = 1/ (1 + abs(vector_cosine_similarity))
                                w2 = 1/ (1+abs(rgb_sim))
                                if (w1 * abs(vector_cosine_similarity) + w2 * abs(rgb_sim) < 0.985 ):
                                    ignore_merge_vector_and_rgb_sp_count += 1
                                    continue
                                if new_neighbour_point_inds!=[]:
                    ...
                    break   
        elif "ablation1" in sampler_args:
            for superpoint_idx in superpoint_inds:
                if budget["click"] > 0:
                    point_inds = components[superpoint_idx]
                    if len(point_inds) >= min_size:
    ...
    # save_path = "/data3/ncl/Semantic3D/fine_tune_data_new/"
    # with open(os.path.join(save_path, cloud_name + "fine_tune_data_3.pkl"),"wb") as f:
    #     pickle.dump(vector_sim,f)
    # with open(os.path.join(save_path,  cloud_name +"fine_tune_lable_3.pkl"),"wb") as f:
...
class TSampler:
    ...
    def sampling(self, model, batch_size, last_round, w, threshold, gcn_gpu):
        ...
        data_path = "/data3/ncl/Semantic3D"
        input_path = "/data3/ncl/Semantic3D/input_0.060"
    ...
    def prediction_original(self, model, total_obj, round_num):
        region_uncertainty = []
        region_class = []
    ...
    def sampling_original(self, model, batch_size, last_round, w, threshold, gcn_gpu):
        budget = {}
        budget["click"] = batch_size
        ...
        region_reference, sorted_inds, prob_class_dict, labeled_region_reference_dict, class_num = self.prediction_original(model=model, total_obj=total_obj, round_num=round_num)
