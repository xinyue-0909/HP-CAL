...
def process_superpoint_files(directory):
    ...
    for filename in os.listdir(sp_directory):
        if filename.endswith(".superpoint"):
            sp_file_path = os.path.join(directory, '0.008', 'superpoint', filename)
            ...
            ply_file_path = os.path.join(directory, 'input_0.040', cloud_name+'.ply')
            ...
            sp_kd_tree_file = os.path.join(directory, '0.008','spKDTree', cloud_name + '_spKDTree.pkl')
            with open(sp_kd_tree_file, 'wb') as f:
                pickle.dump(spKDTree, f)
            sp_neighbours_file = os.path.join(directory, '0.008', 'spNeighbour', cloud_name + '_spNeighbour.pkl')
